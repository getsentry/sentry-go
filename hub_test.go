...
func TestCaptureNilEventWithDisabledClient(t *testing.T) {
	t.Parallel()

	hub := NewHub(nil, NewScope())

	if eventID := hub.CaptureEvent(nil); eventID != nil {
		t.Fatalf("expected disabled client to drop nil event, got %v", eventID)
	}
}
