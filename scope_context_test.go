...
func TestClientFromContext(t *testing.T) {
	globalClient, _ := newCaptureTestClient(t, ClientOptions{})
	localClient, _ := newCaptureTestClient(t, ClientOptions{})
	...
	parentCtx := ContextWithClient(ContextWithScope(context.Background(), NewScope()), localClient)
	childCtx := ContextWithClient(parentCtx, globalClient)
	assert.Same(t, ScopeFromContext(parentCtx), ScopeFromContext(childCtx))
	assert.Same(t, globalClient, ClientFromContext(childCtx))
