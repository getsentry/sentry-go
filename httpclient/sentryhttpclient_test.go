...
func TestIntegration_ResolvesPropagationOptionsFromRequestContext(t *testing.T) {
	...
	for _, sentryClient := range clients {
		ctx, _ := sentry.WithIsolationScope(context.Background())
		ctx = sentry.ContextWithClient(ctx, sentryClient)
		propagation := sentry.NewPropagationContext()
		span := sentry.StartSpan(ctx, "test",
			sentry.ContinueTrace(
				fmt.Sprintf("%s-%s-1", propagation.TraceID, propagation.SpanID),
				"sentry-release=new",
			),
		)
		request, requestErr := http.NewRequestWithContext(span.Context(), http.MethodGet, "https://example.com", nil)
		if requestErr != nil {
			t.Fatal(requestErr)
		}
...
func TestIntegration_PropagationTargetsDoNotDisableInstrumentation(t *testing.T) {
	...
	defer cancel()
	capture := &captureRoundTripper{}
	request, err := http.NewRequestWithContext(requestCtx, http.MethodPost, "https://example.com", strings.NewReader(`{"key":"value"}`))
	if err != nil {
		t.Fatal(err)
	}
	...
	response.Body.Close()
	if request.Context() != requestCtx {
		t.Fatal("caller request context was replaced")
	}
	if request.Body != originalBody {
		t.Fatal("caller request body was replaced")
	}
	if diff := cmp.Diff(originalHeader, request.Header); diff != "" {
		t.Fatalf("caller request headers changed (-want +got):\n%s", diff)
	}
	capturedCtx := capture.requests[0].Context()
	child := sentry.SpanFromContext(capturedCtx)
	if child == nil || child.Op != "http.client" || child.ParentSpanID != transaction.SpanID {
		t.Fatalf("downstream request span = %+v, want http.client child", child)
	}
	if got := capturedCtx.Value(requestContextKey{}); got != "request value" {
		t.Fatalf("downstream context value = %v, want %q", got, "request value")
	}
	cancel()
	select {
	case <-capturedCtx.Done():
	default:
		t.Fatal("downstream request context did not preserve cancellation")
	}
	transaction.Finish()
...
func TestRoundTripDoesNotMutateCallerRequest(t *testing.T) {
	...
	type requestContextKey struct{}
	requestCtx, cancel := context.WithCancel(context.WithValue(span.Context(), requestContextKey{}, "request value"))
	defer cancel()

	request, err := http.NewRequestWithContext(requestCtx, http.MethodGet, "https://example.com/foo", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, roundTripRequest := range roundTripper.requests {
		...
		if child == nil || child.Op != "http.client" || child.ParentSpanID != span.SpanID {
			t.Fatalf("round trip %d span = %+v, want http.client child", i, child)
		}
		if got := roundTripRequest.Context().Value(requestContextKey{}); got != "request value" {
			t.Fatalf("round trip %d context value = %v, want %q", i, got, "request value")
		}
	}
	cancel()
	for i, roundTripRequest := range roundTripper.requests {
