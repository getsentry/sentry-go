...
func ExtractStacktrace(err error) *Stacktrace {
	// NOTE(jamescun): if the error type implements StacktraceGetter we should
	// never attempt reflection, even if the result of GetSentryStracktrace is
	// nil, the supported error packages of this function do not implement
	// StacktraceGetter and are very unlikely to support it in the future
	// given the dependence on this package it would create, so reasonably
	// expected behavior is to return even if nil.
	if get, ok := err.(StacktraceGetter); ok {
		return get.GetSentryStacktrace()
	}
...
// StacktraceGetter is optionally implemented by types that can return a Sentry
// Stacktrace. This is to support generating the Stacktrace at the point where
// a custom error type is created, and capture its callers, rather than rely
// on an outer or more generic exception handler where caller information may
// be less specific.
type StacktraceGetter interface {
	// GetSentryStacktrace returns a Stacktrace that has been generated by
	// NewStacktrace or otherwise converted to a Stacktrace object. Returning
	// nil indicates the type supports Sentry Stacktraces but does not contain
	// one.
	GetSentryStacktrace() *Stacktrace
}
