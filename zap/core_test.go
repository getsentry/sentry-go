...
func TestAddExceptionsFromError(t *testing.T) {
	tests := map[string]struct {
		...
		"Single error": {
			...
			expectedExceptions: []sentry.Exception{
				{Value: "test error", Type: "*sentryzap.customError"},
			},
			expectedProcessed: map[string]struct{}{
		...
		"Nested errors": {
			...
			disableStacktrace: false,
			expectedExceptions: []sentry.Exception{
				{Value: "outer error", Type: "*sentryzap.nestedError"},
				{Value: "inner error", Type: "*sentryzap.customError"},
			},
			expectedProcessed: map[string]struct{}{
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			// Call the method
			result := core.addExceptionsFromError(tc.initialExceptions, tc.initialProcessed, tc.err)
