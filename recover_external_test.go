...
//go:noinline
func recoverScopePanic(ctx context.Context, panicFunc func()) {
...
func TestRecoverUsesPanicOriginStacktrace(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "scope receiver", panicFunc: panicWithError, topFrame: "panicWithError", receiver: true,
		},
		{
			name:      "runtime error",
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.receiver {
				recoverScopePanic(ctx, test.panicFunc)
			} else {
