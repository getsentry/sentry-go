...
func TestBufferDiagnostics(t *testing.T) {
	buffer := NewBuffer[*testItem](DataCategoryError, 2, OverflowPolicyDropNewest)

	// Test empty buffer diagnostics
	diag := buffer.GetDiagnostics()
	if diag.OverflowPolicy != "drop_newest" {
		t.Errorf("Expected overflow policy 'drop_newest', got '%s'", diag.OverflowPolicy)
	}
	if !diag.IsEmpty {
	...
	// Fill buffer and test diagnostics
	buffer.Offer(&testItem{id: 1, data: "test"})
	buffer.Offer(&testItem{id: 2, data: "test"})
