...
func TestFixture_NewContext(t *testing.T) {
	...
	assert.Same(t, f.Client, sentry.ClientFromContext(ctx), "context client")
}
...
func TestSentryFixture_AssertScopeIsolation_Pass(t *testing.T) {
	...
	f := NewFixture(t)
	_, requestScope := sentry.WithIsolationScope(f.Context)
...
func TestSentryFixture_AssertScopeIsolation_DetectsNil(t *testing.T) {
	...
	mock := &testing.T{}
	_, scope := sentry.WithIsolationScope(sentry.ContextWithScope(context.Background(), sentry.GlobalScope()))
	f := &Fixture{T: mock, Scope: scope}
