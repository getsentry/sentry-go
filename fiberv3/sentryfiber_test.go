	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			handler := func(ctx fiber.Ctx) error {
				...
				if tc.useSentry && scope != sentry.ScopeFromContext(ctx.Context()) {
					t.Error("expecting scope helper to use the Fiber context")
				}
...
func TestRequestIsolation(t *testing.T) {
	...
	app.Get("/test", func(ctx fiber.Ctx) error {
		...
		if sentryfiber.GetSpanFromContext(ctx) != sentry.SpanFromContext(ctx.Context()) {
			return errors.New("span helper does not use the Fiber context")
		}
		scopes <- scope
		return nil
	})
