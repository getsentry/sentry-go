...
func TestResolveTraceContextIncludesSamplingDecision(t *testing.T) {
	t.Parallel()
	...
	got, ok := ResolveTraceContext(ctx)
	assert.True(t, ok)
	assert.Equal(t, sentry.TraceID(traceID), got.TraceID)
	assert.Equal(t, sentry.SpanID(spanID), got.SpanID)
	assert.Equal(t, sentry.SampledTrue, got.Sampled)
}
